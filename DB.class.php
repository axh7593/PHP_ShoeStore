...
// get all sale item data from the table 
    function getSaleItems(){
         $data = array();
        if($stmt = $this -> connection -> prepare("SELECT * FROM Project1 WHERE SalePrice > 0")){
          $stmt -> execute();
    ...
    function populatePage($limit,$offset){
            $data =  array();
            if($stmt = $this -> connection -> prepare("SELECT * FROM Project1 WHERE SalePrice = 0 LIMIT $limit OFFSET $offset")){
            $stmt -> execute();
            $stmt -> store_result();
    ...
    function selectAllItems(){
        $data = array();
        if($stmt = $this -> connection -> prepare("SELECT * FROM Project1")){
          $stmt -> execute();
    ...
    function getThisItem($id){
        $data = array();
        if($stmt = $this -> connection -> prepare("SELECT * FROM Project1 WHERE ProductID = $id")){
          $stmt -> execute();
